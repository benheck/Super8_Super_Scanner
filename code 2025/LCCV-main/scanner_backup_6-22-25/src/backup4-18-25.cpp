...
void advanceFilmFrames(float frames, float speed, bool waitForResponse = false) {
...
void capturePhoto() {
    ...
    for (const auto& contour : contours) {
        ...
        if (boundingBox.x < photo.cols / 4 && boundingBox.width > 330 && boundingBox.height > 420) {
            ...
            //cv::rectangle(photo, boundingBox, cv::Scalar(0, 255, 0), 2);
    ...
    // cv::Mat photo4k;
    // cv::resize(croppedPhoto, photo4k, cv::Size(cropWidth / 4, cropHeight / 4)); // Resize the photo to 1/4 of its original size
    // cv::imshow("Captured Photo", photo4k); // Display the captured photo in a window

    cam.startVideo(); // Restart the video stream

    // Save the photo to a file with the frame number

    //Save croppedPhoto to file, filename "frame_" + frameNumber + ".png" and ensure framenumber is 5 digits long:
    ...
    std::string filename = saveFolder.toStdString() + "/frame_" + formattedFrameNumber + ".png";
    cv::imwrite(filename, croppedPhoto); // Save the cropped photo to the selected folder
...
// Button functions
void onNudgeButtonClicked() {
    advanceFilmFrames(0.05f, 250.0f, true); // Move the film forward tiny bits
}

void onNextButtonClicked() {

    advanceFilmFrames(0.7f, 500.0f);    //Move 70% of a frame forward
    //currentState = exitFrame; // Move to the next state
}

void onPrevButtonClicked() {
    //currentState = findCenter;
}
...
void onEnableMotorsClicked() {
    marlin->sendGCode("M17"); // Enable all motors
}

void onDisableMotorsClicked() {
    marlin->sendGCode("M18"); // Disable all motors
}

void onAdvanceFrameClicked() {
    advanceFilmFrames(1.0f + frameDelta, 1000.0f);
}
...
void onAdvance10mmClicked() {
    advanceFilmFrames(10.0f, 1000.0f);
}

void onAdvance50mmClicked() {

    advanceFilmFrames(50.0f, 2000.0f);
}

void onAdvance100mmClicked() {
    advanceFilmFrames(100.0f, 3000.0f);
}

void onStartScanButtonClicked() {
    //TODO: Add check to ensure sprocket is centered-ish
    //Also ensure scan isn't running
...
void onStopScanButtonClicked() {
    ...
    if (currentState == scanning || currentState == frameAdvance) {
        stopScanFlag = true;
    }
...
void updateVideoFeed() {
    ...
    switch(currentState) {
        case setup:
            cv::putText(image, "STATE: SETUP", 
            cv::Point(10, y_offset), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 1);
            y_offset += line_spacing;
            break;
        case nudge:
            cv::putText(image, "STATE: NUDGE",
            cv::Point(10, y_offset), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 1);
            y_offset += line_spacing;
            ...
            break;
        case scanning:
            cv::putText(image, "STATE: SCANNING",
            cv::Point(10, y_offset), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 1);
            y_offset += line_spacing;

            break;
        case frameAdvance:
            cv::putText(image, "STATE: FRAME ADVANCE",
            cv::Point(10, y_offset), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 1);
            y_offset += line_spacing;
    ...
    cv::putText(image, "centerX: " + std::to_string(centerX) + " centerY: " + std::to_string(centerY), 
    cv::Point(10, y_offset), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 1);
    y_offset += line_spacing;

    cv::putText(image, "Frame Delta: " + std::to_string(frameDelta), 
    cv::Point(10, y_offset), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 1);
    y_offset += line_spacing;

    cv::putText(image, "Spool Diameter: " + std::to_string(spool_diameter_mm) + " mm", 
                cv::Point(10, y_offset), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 1);
    y_offset += line_spacing;

    cv::putText(image, "Movement per Turn: " + std::to_string(movement_mm_turn) + " mm", 
                cv::Point(10, y_offset), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 1);
    y_offset += line_spacing;

    cv::putText(image, "Movement Target: " + std::to_string(movement_mm_turn_target) + " mm", 
                cv::Point(10, y_offset), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 1);
    y_offset += line_spacing;

    cv::putText(image, "mm per Frame: " + std::to_string(mmPerFrame) + " mm", 
                cv::Point(10, y_offset), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 1);
    ...
    if (takePhoto) {
        ...
        if (stopScanFlag) {
            ...
            onAdvanceFrameClicked(); // Advance the film by one frame
        }
...
int main(int argc, char *argv[]) {
    ...
    compute_move_per_frame(); // Compute the movement per frame based on the spool diameter

    cameraSetup();

    // Create the main window
    QWidget window;
    window.setWindowTitle("Super 8 Super Scanner");
    ...
    spoolDiameterInput->setPlaceholderText("Starting spool diameter (mm)");
    ...
    // Create buttons
    QPushButton *takePhoto = new QPushButton("Take Photo");
    QPushButton *nudgeButtonForward = new QPushButton("Nudge");
    ...
    QObject::connect(takePhoto, &QPushButton::clicked, capturePhoto);
    QObject::connect(nudgeButtonForward, &QPushButton::clicked, onNudgeButtonClicked);
    ...
    QObject::connect(advance1Button, &QPushButton::clicked, onAdvanceFrameClicked);
    QObject::connect(advance10Button, &QPushButton::clicked, onAdvance10mmClicked);
    QObject::connect(advance50Button, &QPushButton::clicked, onAdvance50mmClicked);
    QObject::connect(advance100Button, &QPushButton::clicked, onAdvance100mmClicked);
    ...
    buttonsLayout->addWidget(nudgeButton);
    ...
    buttonsLayout->addWidget(takePhoto);
    ...
    buttonsSetup->addWidget(selectFolderButton);
