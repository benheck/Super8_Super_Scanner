...
void Options::setApp(LibcameraApp *app) {
    app_ = app;
}

void Options::setAutoExposure(bool lock) {
    if (app_) { // Ensure app_ is not null
        libcamera::ControlList controls(app_->GetCamera()->controls());
        controls.set(libcamera::controls::AeEnable, lock);
        app_->SetControls(controls);
    } else {
