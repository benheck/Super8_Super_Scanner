...
// Add a VideoWriter globally or as a member
...
void startVideoWriter(const std::string& filename, int width, int height, double fps = 30.0) {
    int codec = cv::VideoWriter::fourcc('a', 'v', 'c', '1'); // H.264/MP4 (works if OpenCV built with FFMPEG)
...
void scanSingleFrame() {        //Like updateSetupImage but for scanning
    ...
    if (holeFound) {
        ...
        if (centerY - (scanCropHeightYMax / 2) < 0) { // Check if the centerY is less than half the crop height
            ...
            //     if (!videoWriter.isOpened()) {
            //         startVideoWriter("output.mp4", image.cols, image.rows, 30.0); // 30 FPS example
            //     }
        ...
        switch(state) {
            ...
            case scanning:              //Scanning loop
                if (stopScanFlag) {
                    ...
                    if (highQualityMode == false) {
                        stopVideoWriter(); // Stop the video writer if in high quality mode
                    }
